...
namespace MGroup.MSolve.Core.Discretization.Meshes
{
	/// <summary>
		...
		where TElement : class, ICell<TNode>
	{
		...
		public BidirectionalMesh2D(IReadOnlyList<TNode> vertices, IReadOnlyList<TElement> faces, IDomainBoundary boundary)
		{
			Nodes = vertices;
			Elements = faces;
			...
			foreach (var cell in Elements)
			{
				...
				{
					var exists = connectivity.TryGetValue(vertex, out var vertexCells);
			...
			TElement startingElement = null)
		{
			...
			foreach (var element in Elements) // O(elementsCount)
			{
				var outline = ConvexPolygon2D.CreateUnsafe(element.Nodes.ToCartesianPoints());
				var pos = outline.FindRelativePositionOfPoint(point);
			...
			TElement startingElement = null)
		{
			...
			{
				var outline = ConvexPolygon2D.CreateUnsafe(element.Nodes.ToCartesianPoints());
				var relativePosition = outline.FindRelativePositionOfCircle(circle);
			...
			bool findBoundaryNodes = true, TElement startingElement = null)
		{
			...
			{
				var relativePosition = circle.FindRelativePositionOfPoint(new CartesianPoint(node.X, node.Y));
