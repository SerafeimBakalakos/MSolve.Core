namespace MGroup.MSolve.Geometry.Shapes
{
	...
	public class ConvexPolygon2D
	{
		...
		public static ConvexPolygon2D Create(IReadOnlyList<CartesianPoint> vertices, bool copyInput = false)
		{
			if (copyInput)
			{
		...
		public static ConvexPolygon2D CreateFromCoords(IReadOnlyList<double[]> vertices)
		{
			var points = new CartesianPoint[vertices.Count];
			for (int i = 0; i < vertices.Count; ++i)
			{
				points[i] = new CartesianPoint(vertices[i][0], vertices[i][1]);
			}
