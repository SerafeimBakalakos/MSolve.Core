...
namespace MGroup.MSolve.Numerics.Interpolation
{
	...
	public interface IIsoparametricInterpolation2D
	{
		...
		/// </summary>
		/// <param name="nodes">The nodes of the finite element in the global cartesian coordinate system.</param>
		IInverseInterpolation2D CreateInverseMappingFor(IReadOnlyList<INode> nodes);
		...
		///     element.</param>
		EvalInterpolation2D EvaluateAllAt(IReadOnlyList<INode> nodes, NaturalPoint naturalPoint);
		...
		/// <see cref="EvaluateAllAt(IReadOnlyList{INode}, NaturalPoint)"/> when the integration points of an element are
		...
		///     same.</param>
		IReadOnlyList<EvalInterpolation2D> EvaluateAllAtGaussPoints(IReadOnlyList<INode> nodes, IQuadrature2D quadrature);
