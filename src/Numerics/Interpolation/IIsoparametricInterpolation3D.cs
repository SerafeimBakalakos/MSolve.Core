namespace MGroup.MSolve.Numerics.Interpolation
{
	...
	public interface IIsoparametricInterpolation3D
	{
		...
		/// </summary>
		/// <param name="nodes">The nodes of the finite element in the global cartesian coordinate system.</param>
		IInverseInterpolation3D CreateInverseMappingFor(IReadOnlyList<INode> nodes);
		...
		///     element.</param>
		EvalInterpolation3D EvaluateAllAt(IReadOnlyList<INode> nodes, NaturalPoint naturalPoint);
		...
		/// <see cref="EvaluateAllAt(IReadOnlyList{INode}, NaturalPoint)"/>
		...
		///     same.</param>
		IReadOnlyList<EvalInterpolation3D> EvaluateAllAtGaussPoints(IReadOnlyList<INode> nodes, IQuadrature3D quadrature);
		...
		IReadOnlyList<Matrix> EvaluateNaturalGradientsAtGaussPoints(IQuadrature3D quadrature);
		...
		///     element.</param>
		CartesianPoint TransformNaturalToCartesian(IReadOnlyList<INode> nodes, NaturalPoint naturalPoint);
	}
}
